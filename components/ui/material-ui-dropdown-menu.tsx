...
const useInternalRipple = ({
  disabled = false,
  variant = "item",
}: { disabled?: boolean; variant?: RippleVariant } = {}) => {
...
const M3Styles = () => (
  <style
    ...
    // biome-ignore lint/security/noDangerouslySetInnerHtml: style injection for animation keyframes
        ...
        {React.cloneElement(children as React.ReactElement<any>, {
          children: (
            <>
              ...
              <span
                ref={surfaceRef as React.RefObject<HTMLSpanElement>}
  ...
  const staggeredChildren = React.Children.map(children, (child, index) => {
    if (React.isValidElement(child)) {
      return React.cloneElement(child, {
        style: {
          ...child.props.style,
          "--m3-stagger": id === "main" ? index : index + 1,
        } as React.CSSProperties,
      });
    }
    return child;
